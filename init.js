...
const createImages = async (q /* string */) => {
  let imgs = [];
  let query = `https://www.mylocality.shop/api/search?q=${q}&ext=1`;
  if (lat && lng) {
    query += `&lat=${lat.toFixed(2)}&lng=${lng.toFixed(2)}`;
  } else if (ip) {
    query += `&ip=${ip}`;
  }
  await fetch(query)
    ...
    .then((data) => {
      ...
      hits.slice(0, 3).forEach(({ image, link, priceRange, name }) => {
...
(async function () {
  navigator.geolocation.getCurrentPosition(
    getGeolocationSuccessCallback,
    getGeolocationFailedCallback
  );
